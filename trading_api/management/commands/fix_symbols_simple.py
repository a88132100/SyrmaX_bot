...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('開始簡單修復交易幣種配置...'))
        ...
        try:
            ...
            for symbol in existing_symbols:
                if symbol not in symbols_list:
                    ...
                    try:
                        # 使用filter().delete()而不是直接delete()
                        deleted_count, _ = TradingPair.objects.filter(symbol=symbol).delete()
                        if deleted_count > 0:
                            self.stdout.write(f'✅ 已刪除交易對: {symbol}')
                        else:
                            self.stdout.write(f'⚠️ 交易對 {symbol} 可能已被其他記錄引用，跳過刪除')
                    except Exception as e:
                        self.stdout.write(
                            self.style.WARNING(f'❌ 刪除交易對 {symbol} 時遇到問題: {e}')
                        )
                        # 嘗試手動刪除
                        try:
                            pair = TradingPair.objects.get(symbol=symbol)
                            pair.delete()
                            self.stdout.write(f'✅ 手動刪除交易對成功: {symbol}')
                        except Exception as e2:
                            self.stdout.write(
                                self.style.ERROR(f'❌ 手動刪除也失敗: {e2}')
                            )
