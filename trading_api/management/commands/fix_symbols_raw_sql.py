...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('開始使用原始SQL修復交易幣種配置...'))
        ...
        try:
            ...
            # 使用原始SQL刪除不在目標列表中的交易對
            with connection.cursor() as cursor:
                for symbol in existing_symbols:
                    if symbol not in symbols_list:
                        self.stdout.write(f'嘗試刪除交易對: {symbol}')
                        try:
                            # 首先檢查是否有外鍵引用
                            cursor.execute("""
                                SELECT COUNT(*) FROM trading_api_detailedtradelog 
                                WHERE trading_pair_id IN (
                                    SELECT id FROM trading_api_tradingpair WHERE symbol = %s
                                )
                            """, [symbol])
                            
                            ref_count = cursor.fetchone()[0]
                            if ref_count > 0:
                                self.stdout.write(f'⚠️ 交易對 {symbol} 被 {ref_count} 條詳細日誌引用，先刪除引用記錄')
                                
                                # 刪除引用記錄
                                cursor.execute("""
                                    DELETE FROM trading_api_detailedtradelog 
                                    WHERE trading_pair_id IN (
                                        SELECT id FROM trading_api_tradingpair WHERE symbol = %s
                                    )
                                """, [symbol])
                                ...
                                self.stdout.write(f'✅ 已刪除 {deleted_refs} 條引用記錄')
                            
                            # 現在刪除交易對
                            cursor.execute("DELETE FROM trading_api_tradingpair WHERE symbol = %s", [symbol])
                            deleted_count = cursor.rowcount
                            
                            if deleted_count > 0:
                                self.stdout.write(f'✅ 已刪除交易對: {symbol}')
                            else:
                                self.stdout.write(f'⚠️ 交易對 {symbol} 可能不存在')
                                
                        except Exception as e:
                            self.stdout.write(
                                self.style.ERROR(f'❌ 刪除交易對 {symbol} 時發生錯誤: {e}')
                            )
            ...
            for symbol in symbols_list:
                try:
                    ...
                    if created:
                        ...
                        self.stdout.write(f'ℹ️ 交易對已存在: {symbol}')
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(f'❌ 創建交易對 {symbol} 時發生錯誤: {e}')
                    )
            ...
            # 不拋出異常，讓命令正常結束
            self.stdout.write(self.style.WARNING('修復過程中遇到錯誤，但已嘗試修復部分問題'))
