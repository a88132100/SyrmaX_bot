...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('開始安全修復交易幣種配置...'))
                    ...
                    if symbol not in symbols_list:
                        try:
                            ...
                            if deleted_count > 0:
                                ...
                                self.stdout.write(f'交易對 {symbol} 可能已被其他記錄引用，跳過刪除')
                        except Exception as e:
                            self.stdout.write(
                                self.style.WARNING(f'刪除交易對 {symbol} 時遇到問題: {e}，跳過刪除')
                            )
